...
def get_password_hash(password):
    # Usamos un hash simple para el seed si passlib no está disponible directamente en el script de seed
    # Pero para consistencia con main.py, si podemos usar bcrypt mejor.
    # Por ahora, usemos un placeholder o el mismo método que main.py si es posible.
    # Como seed_data.py es un script independiente, lo haremos compatible.
    try:
...
if not c.execute("SELECT id FROM usuarios WHERE username='admin'").fetchone():
    c.execute("INSERT INTO usuarios (username, hashed_password, role) VALUES (?,?,?)", ("admin", admin_pass, "Admin"))
...
for nom, cuit, dir in empresas_ficticias:
    if not c.execute("SELECT id FROM empresas WHERE cuit = ?", (cuit,)).fetchone():
        email = f"contacto@{nom.lower().replace(' ', '').replace('.', '')}.com"
        tel = "11-4444-5555"
        c.execute("INSERT INTO empresas (nombre, cuit, direccion, activo, jurisdicciones, email, telefono) VALUES (?,?,?,?,?,?,?)",
                  (nom, cuit, dir, 1, "CABA,PBA,PSA-SICOP", email, tel))

# Asegurar que todas las empresas tengan mail/tel
c.execute("UPDATE empresas SET email = 'info@empresa.com' WHERE email IS NULL OR email = ''")
c.execute("UPDATE empresas SET telefono = '11-0000-0000' WHERE telefono IS NULL OR telefono = ''")
